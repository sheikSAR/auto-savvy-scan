...
const PhysicalScoreCard: React.FC<PhysicalScoreCardProps> = ({
  ...
  const getScoreIcon = () => {
    if (physicalScore >= 80) return <CheckCircle className="w-6 h-6 text-green-500" />;
    if (physicalScore >= 50) return <AlertCircle className="w-6 h-6 text-yellow-500" />;
    return <AlertCircle className="w-6 h-6 text-red-500" />;
  };

  const getScoreBarColor = () => {
    if (physicalScore >= 80) return 'bg-gradient-to-r from-green-500 to-green-400';
    if (physicalScore >= 50) return 'bg-gradient-to-r from-yellow-500 to-yellow-400';
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-md border">
      <div className="flex flex-col">
        <div className="flex items-center mb-4">
          ...
          <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100">Physical Condition</h3>
        </div>
        ...
        <div className="mt-4">
          ...
          {fullAnalysisResult && fullAnalysisResult.damages && (
            <div className="space-y-3">
              {Object.entries(fullAnalysisResult.damages).map(([damageType, details]) => (
                details && (
                  <div key={damageType} className="text-sm">
                    <div className="flex justify-between">
                      <span className="font-medium capitalize">{damageType}:</span>
                      <span className={
                        details.type === 'major' ? 'text-red-500' : 'text-yellow-500'
                      }>
                        {details.type}
                      </span>
                    </div>
          ...
          {(!fullAnalysisResult || !fullAnalysisResult.damages || Object.keys(fullAnalysisResult.damages).length === 0) && (
